...
public class Piece {
    ...
    public Piece pawnPromotion(int row, int col, boolean isBlack){
        Scanner s = new Scanner(System.in);

        if((this.character == '\u2659') && col == 7){ // white pawn
            String input = s.nextLine();

            System.out.println("Your pawn made it to the other side! What would you like to promote it to?");
            if ((s.equals("king")) || (s.equals("pawn"))){
                System.out.println("You can turn into any piece but pawn or king.");
            }
            if (s.equals("queen")){
                Piece queen = new Piece('\u2655', row, col, false);
                return queen;
            }
            if (s.equals("rook")){
                Piece rook = new Piece('\u2656', row, col, false);
                return rook;
            }
            if (s.equals("knight")){
                    Piece knight = new Piece('\u2658', row, col, false);
                    return knight;
                }
            if (s.equals("Bishop")){
                Piece bishop = new Piece('\u2657', row, col, false);
                return bishop;
            }
            else{
                System.out.println("Incorrect piece type entered.");
            }
        }

        if ((this.character == '\u265f') && col == 0) { // black pawn
            System.out.println("Your pawn made it to the other side! What would you like to promote it to?");
            String input = s.nextLine();
            if ((s.equals("king")) || (s.equals("pawn"))){
                System.out.println("You can turn into any piece but pawn or king.");
            ...
            if (s.equals("queen")){
                Piece queen = new Piece('\u265b', row, col, true);
                return queen;
            }
            if(s.equals("rook")){
                Piece rook = new Piece('\u265c', row, col, true);
                return rook;
            }
            if(s.equals("knight")){
               Piece knight = new Piece('\u265e', row, col, true);
               return knight;
            }
            if(s.equals("bishop")){
                Piece bishop = new Piece('\u265d', row, col, true);
                return bishop;
            }
            else{
                System.out.println("Incorrect piece type entered.");
        ...
        Piece pawn = new Piece('\u2659', row, col, true);
        return pawn;
        }
